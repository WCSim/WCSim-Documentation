...
\begin{description}
  ...
  Note the above PMT-related methods apply for ID PMTs of the first kind (e.g. 20'' PMTs in the Hyper-K far detector, mPMTs in IWCD). To get information for ID PMTs of the second type, call the same methods with the optional argument \texttt{hybridsecondtype} set to \texttt{true} (the default is \texttt{false} meaning information for the first ID PMT type is returned).
  
  To get information for OD PMTs, there are equivalent methods with ``OD'' in the name.
    \begin{description}
